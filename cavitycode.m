...
% !************************************************************** */
    % Define Pressure Boundary Conditions (2nd Order BC)
    for j=2:jmax-1 
        u(1,j,1)=two*u(2,j,1)-u(3,j,1); % Left Wall
        u(imax,j,1)=two*u(imax-1,j,1)-u(imax-2,j,1); % Right Wall
    end
    for i=2:imax-1
        u(i,1,1)=two*u(i,2,1)-u(i,3,1); % Bottom Wall
        u(i,jmax,1)=two*u(i,jmax-1,1)-u(i,jmax-2,1); % Top Wall
    end
     % Pressure Cornores
    u(1,1,1)=half*(u(2,1,1)+u(1,2,1)); % Bottom Left corner
    u(1,jmax,1)=half*(u(2,jmax,1)+u(1,jmax-1,1));% Top Left Corner
    u(imax,1,1)=half*(u(imax,2,1)+u(imax-1,1,1));% Bottom Right Corner
    u(imax,jmax,1)=half*(u(imax-1,jmax,1)+u(imax,jmax-1,1)); % Top Right Corner
    % Define Velocity Boundary Conditions (All walls are zero except Top side)
    u(1,:,2:3)=0; 
    u(imax,:,2:3)=0;
    u(:,1,2:3)=0;
    u(:,jmax,3)=0;
...
% !************ADD CODING HERE FOR INTRO CFD STUDENTS************ */
% !************************************************************** */
         uvall=zeros(imax,jmax);
        for i=1:imax;
            for j=1:jmax;
                uvall(i,j)=u(i,j,2).^2+u(i,j,3).^2; % Speed Summetion in x&y 
        beta2=max(max(max(uvall(i,j))),(rkappa*vel2ref));
                u_tempmax=max(max(abs(u(i,j,2))));
                v_tempmax=max(max(abs(u(i,j,3))));
                lambda_x=half*(u_tempmax+sqrt(u_tempmax.^2+four*beta2));
                lambda_y=half*(v_tempmax+sqrt(v_tempmax.^2+four*beta2));
                lambda_max=max(lambda_x,lambda_y);
                dtconv=min(dx,dy)/lambda_max;
                dtdiff=fourth*dx*dy/(rmu/rho);
                dt(i,j)=cfl*min(dtconv,dtdiff);
            end
    end
    end
%************************************************************************
...
for i=3:imax-2; 
    for j=3:jmax-2;
        ...
        d4pdx4=(u(i+2,j,1)-four*u(i+1,j,1)+six*u(i,j,1)-four*u(i-1,j,1)+u(i-2,j,1))/dx;
        d4pdy4=(u(i,j+2,1)-four*u(i,j+1,1)+six*u(i,j,1)-four*u(i,j-1,1)+u(i,j-2,1))/dy;
        artviscx(i,j)=-lambda_x*Cx/beta2*d4pdx4;
        artviscy(i,j)=-lambda_y*Cy/beta2*d4pdy4;
    end
end
    for j=3:jmax-2;
             beta2=max(u(2,j,2).^2+u(2,j,3).^2,rkappa*vel2ref);
             lambda_y=half*(abs(u(2,j,3))+sqrt(u(2,j,3).^2+four*beta2));
             d4pdy4=(u(2,j+2,1)-four*u(2,j+1,1)+six*u(2,j,1)-four*u(2,j-1,1)+u(2,j-2,1))/dy;
             artviscy(2,j)=-lambda_y*Cy/beta2*d4pdy4;
             artviscx(2,j)=artviscx(3,j);
             beta2=max(u(imax-1,j,2).^2+u(imax-1,j,3).^2,rkappa*vel2ref);
             lambda_y=half*(abs(u(imax-1,j,3))+sqrt(u(imax-1,j,3).^2+four*beta2));
             d4pdy4=(u(imax-1,j+2,1)-four*u(imax-1,j+1,1)+six*u(imax-1,j,1)-four*u(imax-1,j-1,1)+u(imax-1,j-2,1))/dy;
             artviscy(imax-1,j)=-lambda_y*Cy/beta2*d4pdy4;
             artviscx(imax-1,j)=artviscx(imax-2,j);
    end
    for i=3:imax-2;
           beta2=max(u(i,2,2).^2+u(i,2,3).^2,rkappa*vel2ref);
           lambda_x=half*(abs(u(i,2,2))+sqrt(u(i,2,2).^2+four*beta2));
           d4pdx4=(u(i+2,2,1)-four*u(i+1,2,1)+six*u(i,2,1)-four*u(i-1,2,1)+u(i-2,2,1))/dx;
           artviscx(i,2)=-lambda_x*Cx/beta2*d4pdx4;
           artviscx(i,2)=artviscx(i,3);
           beta2=max(u(i,jmax-1,2).^2+u(i,jmax-1,3).^2,rkappa*vel2ref);
           lambda_x=half*(abs(u(i,jmax-1,2))+sqrt(u(i,jmax-1,2).^2+four*beta2));
           d4pdx4=(u(i+2,jmax-1,1)-four*u(i+1,jmax-1,1)+six*u(i,jmax-1,1)-four*u(i-1,jmax-1,1)+u(i-2,jmax-1,1))/dx;
           artviscx(i,jmax-1)=-lambda_x*Cx/beta2*d4pdx4;
           artviscx(i,jmax-1)=artviscx(i,jmax-2);
    end
    artviscx(2,2)=half*(artviscx(2,3)+artviscx(3,2));
    artviscx(2,jmax-1)=half*(artviscx(3,jmax-1)+artviscx(2,jmax-2));
    ...
    artviscy(2,jmax-1)=half*(artviscy(3,jmax-1)+artviscy(2,jmax-2));
...
% !************************************************************** */
    ...
    for i=2:imax-1;
        for j=2:jmax-1
            temp1=abs(u(i,j,1)-uold(i,j,1))/dtmin;
            res(1)=max(temp1,res(1));
            temp2=abs(u(i,j,2)-uold(i,j,2))/dtmin;
            res(2)=max(temp2,res(2));
            temp3=abs(u(i,j,3)-uold(i,j,3))/dtmin;
            res(3)=max(temp3,res(3));
        end
    end
    conv=sqrt((res(1)^2+res(2)^2+res(3)^2)/3);
...
% !************************************************************** */
    ...
    for i=1:imax
        for j=1:jmax
%             DE1(i,j)=abs(u(i,j,1)-ummsArray(i,j,1));
%             DE2(i,j)=abs(u(i,j,2)-ummsArray(i,j,2));
%             DE3(i,j)=abs(u(i,j,3)-ummsArray(i,j,3));
        end
    end
    for i=1:imax
        for j=1:jmax
            rL1norm(1)=rL1norm(1)+DE1(i,j);
            rL1norm(2)=rL1norm(2)+DE2(i,j);
            rL1norm(3)=rL1norm(3)+DE3(i,j);
        end
    end
    ...
    for i=1:imax
        for j=1:jmax
            rL2norm(1)=rL2norm(1)+DE1(i,j)^2;
            rL2norm(2)=rL2norm(2)+DE2(i,j)^2;
            rL2norm(3)=rL2norm(3)+DE3(i,j)^2;
        end
    end
    ...
    rLinfnorm(1)=max(max(DE1));
    rLinfnorm(2)=max(max(DE2));
    rLinfnorm(3)=max(max(DE3));
end
end
